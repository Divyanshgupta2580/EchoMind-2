...
# Autonomous Twitter AI Agent Framework

**Create fully autonomous AI agents with deep, coherent personalities**

<br/>

[![Python](https://img.shields.io/badge/Python-3.10+-3776AB?style=flat-square&logo=python&logoColor=white)]()
[![FastAPI](https://img.shields.io/badge/FastAPI-0.109+-009688?style=flat-square&logo=fastapi&logoColor=white)]()
[![Twitter API v2](https://img.shields.io/badge/Twitter_API-v2-1DA1F2?style=flat-square&logo=twitter&logoColor=white)]()
[![PostgreSQL](https://img.shields.io/badge/PostgreSQL-14+-4169E1?style=flat-square&logo=postgresql&logoColor=white)]()

### Autonomous posting, intelligent mention handling, and tool execution.

*Synthesize custom character personalities with dynamic tool access, multimodal image generation, and persistent memory.*
...
A modular framework for building **fully autonomous AI agents** on Twitter. Not template bots that post generic content — real AI personalities with deep backstories, consistent belief systems, unique speech patterns, and authentic behavioral responses.

- **Autonomous Posting**: Scheduled and contextual post generation powered by state-of-the-art LLMs.
- **Intelligent Mentions**: Selective reply planning, user interaction history, and contextual tool execution.
- **Multimodal Capabilities**: Dynamic image generation and real-time web search integration.
- **Enterprise-Ready Architecture**: Multi-tier Twitter API management, APScheduler automation, and PostgreSQL persistence.
...
2. **Configure Environment Variables**
   ...
   - `OPENROUTER_API_KEY`: OpenRouter API key for LLM and image generation
   - `TWITTER_API_KEY`, `TWITTER_API_SECRET`, `TWITTER_ACCESS_TOKEN`, `TWITTER_ACCESS_SECRET`, `TWITTER_BEARER_TOKEN`: Twitter API v2 credentials
   - `DATABASE_URL`: PostgreSQL connection string

3. **Customize Personality & System Prompt**
   Edit files in `config/personality/`:
   - `backstory.py`: Define agent origin and backstory
   - `beliefs.py`: Core values, opinions, and topics of interest
   - `instructions.py`: Communication style, tone, and formatting constraints
   - `sample_tweets.py`: Representative tweets in character

4. **Run the Agent**
   ```bash
   python main.py
   ```
   The FastAPI server and scheduler will start automatically on port 8080.

### Requirements

- **Python 3.10+** — Modern async runtime
- **Twitter API v2** — Read/write access for tweets and mentions
- **OpenRouter API** — Multi-model LLM access (Claude, GPT, Gemini)
- **PostgreSQL** — Persistent store for posts, mentions, and agent actions
